...
static const char * const gc5035_supplies[] = {
	/*
	 * This board only exposes a single GPIO-gated "avdd" rail via
	 * INT3472 (see skl_int3472_regulator_map_supplies in
	 * drivers/platform/x86/intel/int3472/clk_and_regulator.c), which
	 * gates power for I/O, digital core and analog domains together.
	 * Request the same supply under all the names this driver uses so
	 * every regulator_get resolves to that single real regulator
	 * instead of falling back to a dummy.
	 */
	"avdd",		/* Digital core power (aliased to the shared rail) */
	"avdd",		/* Analog power (aliased to the shared rail) */
};
...
static void gc5035_set_power(struct gc5035 *gc5035, int on)
{
	...
	/*
	 * Set power pin. On mainline INT3472 the "power-enable" GPIO type
	 * is not exposed to the sensor as a plain GPIO consumer - INT3472
	 * consumes it itself to back a regulator (named "avdd"), so power
	 * this on/off through the regulator instead of a raw GPIO toggle.
	 */
	if (power->pwren_gpio) {
		gpiod_set_value_cansleep(power->pwren_gpio, on);
	} else if (!IS_ERR_OR_NULL(gc5035->iovdd_supply)) {
		int ret;

		if (on)
			ret = regulator_enable(gc5035->iovdd_supply);
		else
			ret = regulator_disable(gc5035->iovdd_supply);
		if (ret)
...
static int gc5035_probe(struct i2c_client *client)
{
	...
	/* Get the avdd regulator before any power sequencing needs it */
	gc5035->iovdd_supply = devm_regulator_get(dev, "avdd");
